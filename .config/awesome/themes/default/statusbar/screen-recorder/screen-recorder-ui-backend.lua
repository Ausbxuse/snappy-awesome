...
local widget_icon_dir = config_dir .. 'icons/screen-recorder/'

-- The screen-recorders scripting 
local screen_rec_backend = require(
                             'themes.default.statusbar.screen-recorder.screen-recorder-scripts')

-- The screen-recorder's UI
local screen_rec_ui = require(
                        'themes.default.statusbar.screen-recorder.screen-recorder-ui')
...
sr_resolution_tbox:set_markup('<span foreground="#FFFFFF66">' ..
                                sr_user_resolution .. "</span>")
sr_offset_tbox:set_markup('<span foreground="#FFFFFF66">' .. sr_user_offset ..
                            "</span>")
...
local emphasize_active_tbox = function()
  if sr_active_tbox == 'res_tbox' then
    sr_resolution_box.border_width = dpi(1/1.645)
    sr_resolution_box.border_color = '#F2F2F2AA'
  elseif sr_active_tbox == 'offset_tbox' then
    sr_offset_box.border_width = dpi(1/1.645)
...
local settings_updater = awful.keygrabber {
  ...
  keypressed_callback = function(self, mod, key, command)
    if key == 'BackSpace' then delete_key() end
  end,
  keyreleased_callback = function(self, mod, key, command)
    ...
    if key:match('%d') or key == 'x' or key == ',' then write_to_textbox(key) end

  end
}

-- Textboxes
sr_resolution_tbox:buttons(gears.table.join(
...
local sr_navigation_reset = function()
  if sr_screen then
    ...
    local recorder_settings =
      sr_screen:get_children_by_id('recorder_settings')[1]
...
sr_settings_button:buttons(gears.table.join(
                             awful.button({}, 1, nil, function()
    if not status_recording and not status_countdown then sr_navigation() end
  end)))
...
local screen_rec_close = function()
  ...
  for s in screen do s.recorder_screen.visible = false end
  settings_updater:stop()
  sr_navigation_reset()
  sr_screen = nil
end

sr_close_button:buttons(gears.table.join(
                          awful.button({}, 1, nil,
                                       function() screen_rec_close() end)))
...
sr_toggle_button:buttons(gears.table.join(
                           awful.button({}, 1, nil, function()
    for s in screen do s.recorder_screen.visible = false end
...
local sr_recording_stop = function()
  ...
  -- Manipulate UIs
  sr_toggle_imgbox:set_image(widget_icon_dir .. 'start-recording-button' ..
                               '.svg')
...
awesome.connect_signal('widget::screen_recorder',
                       function() sr_recording_stop() end)

awesome.connect_signal('widget::screen_recorder:toggle', function()
  if status_recording then
    sr_recording_stop()
    sr_toggle_button.visible = false
  else
    sr_toggle_button.visible = true
    status_audio = true
    sr_recording_start()
  end
end)
...
sr_audio_button:buttons(gears.table.join(
                          awful.button({}, 1, nil,
                                       function() sr_audio_mode() end)))
