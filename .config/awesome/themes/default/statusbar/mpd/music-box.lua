...
screen.connect_signal('request::desktop_decoration', function(s)
  ...
  local ui_content = require('themes.default.statusbar.mpd.content')
    ...
    {
      {
        ...
        expand = 'none',
        spacing = dpi(8),
        {s.album, bottom = dpi(5), widget = wibox.container.margin},
        {
          layout = wibox.layout.fixed.vertical,
          {
            spacing = dpi(4),
    ...
    if music_box.visible then
      ...
      if type == 'keyboard' then
        ...
        awful.placement.top_right(music_box, {
          margins = {top = dpi(5), right = dpi(music_box_x or 5)},
          honor_workarea = true
        })
      else
  ...
  awesome.connect_signal('widget::music',
                         function(type) toggle_music_box(type) end)
  s.backdrop_music:buttons(awful.util.table.join(
                             awful.button({}, 1, nil,
                                          function() toggle_music_box() end)))
end)

music_func.toggle_music_box = toggle_music_box

local mpd_updater = require(
                      'themes.default.statusbar.mpd.mpd-music-updater')
