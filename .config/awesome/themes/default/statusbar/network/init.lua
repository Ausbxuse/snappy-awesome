...
local return_button = function()
  ...
  local widget_button = wibox.widget {
    {widget, margins = dpi(7 / 1.645), widget = wibox.container.margin},
    widget = clickable_container
  }

  widget_button:buttons(gears.table.join(
                          awful.button({}, 1, nil, function()
      awful.spawn("st -e nmtui", false)
    end)))
  ...
  local check_internet_health = [=[
	...
	vpn_status=$(vpn | awk '{print $3}' | head -n 1)
	if [ $vpn_status = "off" ];
	then
		if [ $status_ping -eq 0 ];
		then
  ...
  local update_reconnect_startup =
    function(status) reconnect_startup = status end

  -- Update tooltip
  local update_tooltip =
    function(message) network_tooltip:set_markup(message) end
    ...
    local update_wireless_data = function(strength, healthy)
      awful.spawn.easy_async_with_shell([[
        ...
        local message = 'Connected to: <b>' .. (essid or 'Loading...*') ..
                          '</b>\nWireless Interface: <b>' .. "wlp107s0" ..
                          '</b>\nWiFi-Strength: <b>' .. tostring(wifi_strength) ..
                          '%' .. '</b>\nBit rate: <b>' .. tostring(bitrate) ..
                          '</b>'
    ...
    local update_wireless_icon = function(strength)
      awful.spawn.easy_async_with_shell(check_internet_health, function(stdout)
        local widget_icon_name = 'wifi-strength'
				if not stdout:match('VPN connected') then
					...
					widget.icon:set_image(widget_icon_dir .. widget_icon_name .. '.svg')
						...
						widget_icon_name = widget_icon_name .. '-' .. tostring(strength) .. '-lock'
						update_wireless_data(wifi_strength_rounded, true)
						widget.icon:set_image(widget_icon_dir .. widget_icon_name .. '.svg')
				end
      end)
  ...
  local update_wired = function()
    ...
    local notify_connected = function()
      local message = 'Connected to internet with <b>\"' .. "lo" .. '\"</b>'
      local title = 'Connection Established'
      local app_name = 'System Notification'
    ...
    awful.spawn.easy_async_with_shell(check_internet_health, function(stdout)
      ...
      if stdout:match('Connected but no internet') then
        ...
        update_tooltip('<b>Connected but no internet!</b>' ..
                         '\nEthernet Interface: <b>' .. "lo" .. '</b>')
      else
        update_tooltip('Ethernet Interface: <b>' .. "lo" .. '</b>')
        if startup or reconnect_startup then
          awesome.emit_signal('system::network_connected')
          notify_connected()
  ...
  local update_disconnected = function()
    ...
    local notify_wireless_disconnected =
      function(essid)
